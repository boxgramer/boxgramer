...
<div class="flex w-full flex-col items-center justify-center gap-10 p-2 md:p-20 text-center relative">
    <div class=" jumping-button rounded-full h-10 w-10 md:h-24 md:w-24 flex items-center justify-center bg-green-200 fixed right-5 bottom-5 z-30">
        <div class=" tooltip bg-gray-800 w-[170px] md:w-[270px] text-white text-xs md:text-xl text-left rounded-lg px-2 py-1 absolute right-1/2 transform  translate-x-[-15%] md:translate-x-[-20%]">
...
feel free to ask me anything
  ...
  <!-- Email Link -->
    ...
    {{-- foto profile --}}
    <div class="h-24 w-24 overflow-hidden rounded-full border-4 border-gray-300">
        <img src="{{ Vite::asset('resources/images/imagerimba.jpeg') }}" alt="profile picture"
...
{{$this->summary}}
    ...
    <div class="flex w-full flex-col items-start justify-center gap-5">
        ...
        <div class="mb-8 w-full">
            <h2 class="mb-4 text-2xl font-bold uppercase text-gray-800">Career</h2>
            <div class="space-y-4">
                <!-- Career Item -->
                @foreach ($this->carrers as $carrer)
                    <div class="flex flex-col justify-end rounded-lg bg-white p-4 text-left shadow sm:flex-row">
                        <div class="flex-grow">
                            <h3 class="text-xl font-semibold text-gray-800">{{ $carrer['nama'] }}</h3>
                            <p class="text-gray-600">{{ $carrer['company'] }}</p>
                            <p class="text-sm text-gray-500">{{ $carrer['time'] }}</p>
                            <p class="text-sm text-gray-500">{{ $carrer['jobdesk'] }}</p>
                            <p class="text-sm text-gray-500">{{ $carrer['status'] }}</p>
                        </div>
                        <div class="flex-none">
                            @foreach ($carrer['techstack'] as $tech)
                                <span
                                    class="rounded-full bg-blue-500 px-3 py-1 text-sm text-white">{{ $tech }}</span>
                            @endforeach
                        </div>
                    </div>
                @endforeach
        ...
        {{-- project right --}}
        <div class="mb-8 w-full">
            <h2 class="mb-4 text-2xl font-bold uppercase text-gray-800">Projects</h2>
            <div class="space-y-4">
                <!-- Project Item -->
                @foreach ($this->projects as $project)
                    <div class="rounded-lg bg-white p-1 md:p-4 text-left shadow">
                        <div class="flex justify-between">
                            <h3 class="text-xl font-semibold text-gray-800">{{ $project['nama'] }}</h3>
                            <span
                                class="rounded-full bg-green-500 px-3 py-1 text-sm text-white h-7">{{ $project['status'] }}</span>
                        </div>
                        <div class="flex justify-between items-start mt-5">
                            <div>
                                <p class="text-gray-600">{{ $project['jobdesk'] }}</p>
                                <h5 class=" font-bold">feature</h5>
                            ...
                            <div class="flex flex-col md:flex-row gap-1">
                                @foreach ($project['techstack'] as $tech)
                                    <span
                                        class="rounded-full bg-gray-200 px-3 py-1 text-sm text-gray-800">{{ $tech }}</span>
                                @endforeach
                            </div>
    ...
    <div class="mx-w-md mx-auto p-4 text-center">
        ...
        <h2 class="mb-4 text-2xl font-bold text-gray-800">Contact</h2>
        <p class="mb-6 text-gray-600">Feel free to connect with me on LinkedIn, check out my projects on GitHub, or send
            me an email to boxgramer@gmail.com.</p>
        ...
        <div class="flex justify-center space-x-4 mb-10">
            ...
            <a href="https://linkedin.com/in/yourprofile" target="_blank" rel="noopener noreferrer"
                ...
                <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M22.23 0H1.77C.79 0 0 .774 0 1.73v20.54C0 23.226.79 24 1.77 24h20.46c.98 0 1.77-.774 1.77-1.73V1.73C24 .774 23.21 0 22.23 0zM7.12 20.452H3.56V9h3.56v11.452zM5.34 7.457a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM20.452 20.452h-3.56v-5.908c0-1.408-.027-3.222-1.963-3.222-1.964 0-2.264 1.533-2.264 3.116v6.014h-3.56V9h3.42v1.566h.05c.477-.904 1.636-1.854 3.369-1.854 3.603 0 4.268 2.37 4.268 5.454v6.286h-.001z" />
                </svg>
            </a>
